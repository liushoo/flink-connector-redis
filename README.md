...
| value.data.structure  | column | String | column: value值来自某一字段 (如, set: key值取自DDL定义的第一个字段, value值取自第二个字段)<br/> row: 将整行内容保存至value并以'\01'分割<\br>JSON格式保存 |
| value.data.separate   | column | String | value.data.structur为row时,指定字段分隔符,                                                                             |                                                                                                               |
| keyPrefix             |   ""   | String | 键Key前缀,sink时保存key前缀                                                                                           |
